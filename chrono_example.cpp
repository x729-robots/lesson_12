...
template <typename T> void make_random_shuffle(std::vector<T>& values) {
...
void run_test_suite(size_t iteration_count) {
    ...
    // std::sort algorithm measuring
    {
        ...
        auto milliseconds =
            std::chrono::duration_cast<std::chrono::milliseconds>(end_time -
                                                                  start_time);
        std::cout << "std::sort duration = " << milliseconds.count() << "ms"
                  << std::endl;
        std::cout << "std::sort avg = "
                  << milliseconds.count() / iteration_count << "ms"
                  << std::endl;
    }
    ...
    // C-style quick-sort algorithm measuring
    {
        ...
        for (size_t i = 0; i < iteration_count; ++i) {
            std::vector<int> test_data = values;
            std::qsort(test_data.data(), test_data.size(), sizeof(int),
        ...
        auto milliseconds =
            std::chrono::duration_cast<std::chrono::milliseconds>(end_time -
                                                                  start_time);
        std::cout << "qsort duration = " << milliseconds.count() << "ms"
                  << std::endl;
        std::cout << "qsort avg = " << milliseconds.count() / iteration_count
                  << "ms" << std::endl;
    }
}
